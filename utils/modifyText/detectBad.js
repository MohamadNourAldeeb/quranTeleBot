...
let removeFrequency = (text) => {
  const words = text.split(" ");
  const uniqueWords = words.map((word) => Array.from(new Set(word.split(""))));
...
let checkBadEveryWord =(text)=>{
  const words = text.split(/\s+/);
  words.every((word) => {
    const checkResult = detectedBad(word);
    if (checkResult == "error" && word !== "ال") {
      return "error"
    }
   return text
...
module.exports=  {detectedBad,checkBadEveryWord};
